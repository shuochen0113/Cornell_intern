...
// Computes the normalization factor (zeta) for Zipfian distributions.
...
void gpu_layout_kernel(int iter, cuda::layout_config_t config, curandStateCoalesced_t *rnd_state, double eta, double *zetas, 
    ...
    // If cooling is enabled, n2 is chosen from nearby steps in the path, using a Zipfian distribution
    // Else, n2 is chosen randomly
